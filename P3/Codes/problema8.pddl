(define (problem Ej8Prob)
    ...
    (:init
        ...
        ; Coste en tiempo del plan
        (= (costeTiempo) 0)

        ; Coste en tiempo de construir unos Barracones y un Extractor
        (= (costeTiempoEdificio Barracones) 50)
        (= (costeTiempoEdificio Extractor) 20)

        ; Coste en tiempo para reclutar las unidades
        (= (costeTiempoReclutarUnidad VCE) 10)
        (= (costeTiempoReclutarUnidad Marine) 20)
        (= (costeTiempoReclutarUnidad Soldado) 30)

        ; Coste en tiempo de mover las distintas unidades
        (= (costeTiempoMover VCE) 1)
        (= (costeTiempoMover Marine) 5)
        (= (costeTiempoMover Soldado) 10)

        ; Coste en tiempo de recolectar los distintos recursos
        (= (costeTiempoRecolectar Mineral) 10)
        (= (costeTiempoRecolectar Gas) 5)
    
        ; Caminos que tienen un coste de 20
        (tieneCosteMayor1 loc22 loc23)
        (tieneCosteMayor1 loc23 loc22)
        (tieneCosteMayor1 loc42 loc43)
        (tieneCosteMayor1 loc43 loc42)

        ; Caminos que tienen un coste de 40
        (tieneCosteMayor2 loc44 loc15)
        (tieneCosteMayor2 loc15 loc44)

    )
    (:goal (and
        ; Goal constuir barracones en loc33
        ; tener marine1 loc31 otro marine en loc24
        ; tener soldado en loc15
        (en Barracones1 loc33)
        (en Marine1 loc31)
        (en Marine2 loc24)
        (en Soldado1 loc15) 

        ; Restriccion de coste en tiempo
        (< (costeTiempo) 558)
    ))
)